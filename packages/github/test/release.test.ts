...
describe('release', () => {
	it('should throw error when creating release without token', async() => {
		...
		try {
			await expect(createRelease('1.0.0', 'test-package', 'v1.0.0', options)).rejects.toThrow('GITHUB_TOKEN');
		}
		finally {
	...
	it('should throw error when creating release without repo', async() => {
		...
		try {
			await expect(createRelease('1.0.0', 'test-package', 'v1.0.0', options)).rejects.toThrow('repository not configured');
		}
		finally {
			if (typeof originalToken === 'string') {
				process.env.GITHUB_TOKEN = originalToken;
