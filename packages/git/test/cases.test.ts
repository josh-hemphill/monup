...
import { getCommits, getCurrentBranch, getFirstCommit, getGitHubRepo, getLastTag, getTags, isPrerelease } from '../src/index.ts';
...
describe('git package - shell command test cases', () => {
	...
	it('should get commits from tag to HEAD', async() => {
		const tags = await getTags(testRepoDir);
		expect(tags.length).toBeGreaterThan(0);
		const lastTag = tags[tags.length - 1];
		const commits = await getCommits(lastTag, undefined, mockPackages, testRepoDir);
		expect(Array.isArray(commits)).toBe(true);
		expect(commits.length).toBeGreaterThanOrEqual(0);
	});

	it('should get commits in range', async() => {
		const tags = await getTags(testRepoDir);
		expect(tags.length).toBeGreaterThanOrEqual(2);
		const fromTag = tags[tags.length - 2];
		const toTag = tags[tags.length - 1];
		const commits = await getCommits(fromTag, toTag, mockPackages, testRepoDir);
		expect(Array.isArray(commits)).toBe(true);
		expect(commits.length).toBeGreaterThanOrEqual(0);
	});
	...
	it('should get last tag with default filter', async() => {
		const lastTag = await getLastTag(undefined, undefined, testRepoDir);
	...
	it('should get last tag with custom filter', async() => {
		const lastTag = await getLastTag((tag) => tag.startsWith('v'), undefined, testRepoDir);
		expect(lastTag).toBeDefined();
		expect(typeof lastTag).toBe('string');
		const tag = lastTag as string;
	...
	it('should get last tag with template', async() => {
		const lastTag = await getLastTag(undefined, 'v%s', testRepoDir);
		expect(lastTag).toBeDefined();
		expect(typeof lastTag).toBe('string');
		const tag = lastTag as string;
		expect(tag.length).toBeGreaterThan(0);
